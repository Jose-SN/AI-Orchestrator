"""LangChain agent setup for conversational orchestration."""
...
from app.domain.prompts.system_prompts import ORCHESTRATOR_SYSTEM_PROMPT
...
class OrchestratorAgent:
    """
    LangChain ReAct agent with dynamic, permission-filtered tool calling.

    The agent NEVER accesses databases — it only invokes registered HTTP tools.
    """
    ...
    async def run(
        ...
        *,
        ...
        if not tools:
            logger.warning("no_tools_available", user_id=user.user_id)
            return {
                "response": (
    ...
    def _extract_result(result: dict[str, Any]) -> tuple[str, list[dict[str, Any]]]:
        """Parse LangGraph agent result into response text and tool invocations."""
