...
class IAMClient(BaseHTTPClient):
    ...
    async def validate_token(self, token: str) -> dict[str, Any]:
        try:
            ...
            raise IAMServiceError(f"Token validation failed: HTTP {exc.response.status_code}") from exc
    ...
    async def get_user_permissions(self, token: str) -> UserContext:
        try:
            ...
            return UserContext(
                ...
                module_permissions=data.get("modulePermissions") or data.get("module_permissions", []),
            )
        except (AuthenticationError, AuthorizationError):
            raise
        except httpx.HTTPStatusError as exc:
            raise IAMServiceError(f"Permission lookup failed: HTTP {exc.response.status_code}") from exc
        except httpx.RequestError as exc:
            raise IAMServiceError(f"IAM service unreachable: {exc}") from exc
